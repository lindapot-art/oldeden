#!/usr/bin/env node
// 👑 KING'S SUPREME EVE ONLINE COMPREHENSIVE TESTING SYSTEM
...
async function findGamePort() {
    for (let port of [TEST_CONFIG.gameUrl.split(':')[2], ...TEST_CONFIG.fallbackPorts]) {
        try {
            const testUrl = `http://localhost:${port}`;
            const response = await fetch(testUrl);
            if (response.ok) {
                console.log(`🌐 Game server found on port ${port}`);
                return testUrl;
            }
        } catch (e) {
            continue;
    ...
    throw new Error('❌ No game server found on any port');
}
...
async function testEVEDefenseSystems() {
    ...
    await page.keyboard.press('KeyS'); // Shield booster activation
...
async function testDroneSystems() {
    ...
    await page.keyboard.press('Digit1'); // Light drones
    await page.keyboard.press('KeyD');   // Deploy
    ...
    await page.keyboard.press('KeyR');   // Recall
    await page.waitForTimeout(2000);
...
async function testMiningOperations() {
    ...
    await page.keyboard.press('KeyM'); // Mining mode
    await page.waitForTimeout(3000);
    await captureScreenshot('mining', 'mining_active');
    
    // Harvest ore
    for (let i = 0; i < 5; i++) {
        await page.click(960, 540); // Click center screen
        await page.waitForTimeout(2000);
...
async function testTradingSystems() {
    ...
    await page.keyboard.press('KeyT'); // Trading mode
    await page.waitForTimeout(2000);
    await captureScreenshot('trading', 'trading_interface');
    
    // Simulate ore selling
    await page.click(1200, 600); // Click sell button area
    await page.waitForTimeout(2000);
...
async function verify3DNPCModels() {
    ...
    const npcInfo = await page.evaluate(() => {
        // Check for NPCs and their 3D models
        const npcs = [];
    ...
    try {
        ...
        while (screenshotsTaken < TEST_CONFIG.screenshotCount) {
            ...
            for (let i = 0; i < 3 && screenshotsTaken < TEST_CONFIG.screenshotCount; i++) {
                await new Promise(resolve => setTimeout(resolve, TEST_CONFIG.intervalSeconds * 1000));
