...
async function testNewGameFlow() {
  ...
  try {
    ...
    console.log('   - Found ' + (newGameMatches ? newGameMatches.length : 0) + ' "NEW GAME" text occurrences');
    console.log('   - Found ' + (buttonMatches ? buttonMatches.length : 0) + ' NEW GAME button elements');
    ...
    console.log('');
    console.log('3. Checking for resurrection modal code...');
    ...
    console.log('   - Found ' + (resurrectionMatches ? resurrectionMatches.length : 0) + ' "resurrection" references');
    console.log('   - Found ' + (modalMatches ? modalMatches.length : 0) + ' resurrection modal references');
    
    // Step 4: Look for character creation elements
    console.log('');
    console.log('4. Checking for character creation elements...');
    const charCreateMatches = html.match(/character.*creation|create.*character/gi);
    const nameInputMatches = html.match(/<input[^>]*name[^>]*>/gi);
    
    console.log('   - Found ' + (charCreateMatches ? charCreateMatches.length : 0) + ' character creation references');
    console.log('   - Found ' + (nameInputMatches ? nameInputMatches.length : 0) + ' name input fields');
    
    // Step 5: Try to find any JavaScript that handles NEW GAME
    console.log('');
    console.log('5. Analyzing JavaScript for NEW GAME handling...');
