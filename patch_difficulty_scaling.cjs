...
console.log('⚖️ Implementing Dynamic Difficulty Scaling System...');
...
const DifficultySystem = {
  ...
  updateDifficulty() {
    ...
    if (Math.abs(adjustment) > 0.01) {
      ...
      console.log(\`[DifficultySystem] Adjusted to \${newDifficulty.toFixed(2)} (performance: \${performance.toFixed(2)})\`);
    }
  },
...
function updateDifficultySystem() {
  // Update playtime
  DifficultySystem.metrics.playtimeMinutes = (performance.now() - gameStartTime) / 60000;
...
`);
  ...
  // Add difficulty update to game loop
  const gameLoopPattern = `      updateTargetingSystem();
      if (gameLoop._errCount) gameLoop._errCount = 0;`;
      
  const gameLoopWithDifficulty = cr(`      updateTargetingSystem();
  ...
  // Integrate difficulty with enemy creation (find enemy spawn/creation code)
  const enemySpawnPattern = html.indexOf('c.enemies.push(');
  
  if (enemySpawnPattern !== -1) {
    // Find the line containing enemy push
    const lines = html.split('\r\n');
    let enemyPushLineIndex = -1;
    
    for (let i = 0; i < lines.length; i++) {
      if (lines[i].includes('c.enemies.push(') && lines[i].includes('enemy')) {
        enemyPushLineIndex = i;
        break;
      }
    }
    
    if (enemyPushLineIndex !== -1) {
      // Insert difficulty application after enemy push
      const insertLine = enemyPushLineIndex + 1;
      const newLines = [
        ...lines.slice(0, insertLine),
        '        // Apply dynamic difficulty scaling',
        '        if (c.enemies.length > 0) DifficultySystem.applyToEnemy(c.enemies[c.enemies.length - 1]);',
        ...lines.slice(insertLine)
      ];
      
      html = newLines.join('\r\n');
      console.log('✅ Integrated difficulty scaling with enemy spawning');
    }
  }
