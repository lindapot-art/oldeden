// FACTION WARFARE MECHANICS SYSTEM
...
console.log('⚔️ Implementing Faction Warfare Mechanics...');
...
const FactionSystem = {
  ...
  generateFactionMission(factionId) {
    const faction = this.factions[factionId];
    if (!faction) return null;
    
    const rep = this.reputation[factionId];
    if (rep < -60) return null; // Too hostile for missions
    
    const missionTypes = [
      {
        type: 'eliminate',
        target: faction.enemies[Math.floor(Math.random() * faction.enemies.length)],
        count: 3 + Math.floor(rep / 20),
        reward: 50 + rep,
        description: 'Eliminate enemy ships'
      },
      {
        type: 'patrol',
        area: faction.territory,
        duration: 60000, // 1 minute
        reward: 30 + rep,
        description: 'Patrol faction territory'
      }
    ];
...
function showFactionStatus() {
  ...
  for (const [id, faction] of Object.entries(fs.factions)) {
    ...
    const color = fs.getStandingColor(rep);
...
`);
  ...
  // Integrate faction system with enemy kills
  // Find the enemy kill handling code
  const killPattern = 'onKill(enemy)';
  if (html.includes(killPattern)) {
    const killHandlingPattern = `      DifficultySystem.onKill(enemy);`;
    const killWithFaction = cr(`      DifficultySystem.onKill(enemy);
      FactionSystem.onEnemyKilled(enemy);`);
    
    html = safeReplace(html, killHandlingPattern, killWithFaction, 'faction kill integration');
